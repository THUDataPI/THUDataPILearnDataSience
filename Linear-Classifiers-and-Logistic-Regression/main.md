...
线性回归假设结果和特征满足线性关系，即![](http://latex.codecogs.com/gif.latex?h_{\Theta} (x)=\Theta{_0}+\Theta{_1}x_1+\Theta{_2}x_2+\Theta{_3}x_3)  ，为连续模型，容易受噪声影响。
逻辑回归本质上是线性回归，只是在特征到结果的映射上添加一层函数映射，即![](http://latex.codecogs.com/gif.latex?z=h_{\Theta} (x)=\Theta{_0}+\Theta{_1}x_1+\Theta{_2}x_2+\Theta{_3}x_3)，![](http://latex.codecogs.com/gif.latex?g(z)=\frac{1}{1+e^{-z}}) ，添加映射后，连续模型变为二元模型，受噪声影响变小。  
